...
const business = async (userId, payload) => {
    try {
        ...
        addresses.postal_code = payload.addresses?.registered?.postalCode ? payload.addresses?.registered?.postalCode : undefined;
        ...
        const account = await razorpayService.createLinkedAccount(payload);
...
const stakeholder = async (userId, payload) => {
    try {
        ...
        addresses.postal_code = payload.addresses?.residential?.postalCode ? payload.addresses?.residential?.postalCode : undefined;
        addresses.country = payload.addresses?.residential?.country ? payload.addresses?.residential?.country : undefined;
        if (Object.values(addresses).find(obj => obj !== undefined)) {
            accountDetails.profile.addresses = { residential: { ...addresses } };
        }

        logger('debug', 'Registering stakeholder to razorpay:', stakeholderDetails);
        const stakeholder = await razorpayService.createStakeholder(paymentGatewayDetails.accountId, payload);
