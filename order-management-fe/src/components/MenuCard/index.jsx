...
function MenuCard({ data = {}, count = [], currentPage = 0, name = '', handleClick = () => { }, handleOnChange = () => { } }) {
    ...
    const CoverView = ({ type }) => {
        ...
        return (
            <>
                ...
                <h2 className="fw-bold mt-2" style={{ color: '#FDFD96' }}>{name}</h2>
    ...
    const MenuItemView = ({ type, data }) => {
        ...
        return (
            data.map((item, index) => (
                <div
                    className="col-12 d-flex align-items-center mt-2 list-view"
                    key={`${index}-${item.name}`}
                >
